...
class AgendamentoActions {
  ...
  static void executeActionForStatus(
      BuildContext
          context, // Contexto onde a ação foi iniciada (geralmente o Modal)
      String status,
    ...
    void successWithStatus(String newStatus) =>
        onComplete(true, newStatus: newStatus);
    ...
    print(
        "AgendamentoActions: Executando ação para status '$status' na aba ROL $currentTabIndex (Agendamento ID: ${agendamento.idAgendamento})");
    ...
    switch (status) {
      case 'solicitado':
        if (currentTabIndex == 0) {
          ...
          // Prestador pode gerenciar (Aceitar/Recusar)
          gerenciarSolicitacao(context, agendamento, successWithStatus, failure,
              closeLocalDialog);
        }
        break;
      ...
      case 'aguardando':
        if (currentTabIndex == 0) {
          ...
          // Prestador pode verificar pagamento
      ...
      case 'confirmado': // Status após pagamento bem-sucedido
        if (currentTabIndex == 0) {
          // Cliente: Ação padrão é apenas ver detalhes
          print(
              "Ação Cliente 'confirmado': Nenhuma ação backend, chamando simpleSuccess.");
          // Não precisa fechar o modal principal aqui, o onComplete fará isso se necessário
          simpleSuccess();
        } else {
          // Prestador: Pode marcar como concluído
      ...
      case 'concluído':
        if (currentTabIndex == 0) {
          ...
          avaliarServico(
              context, agendamento, simpleSuccess, failure, closeLocalDialog);
        } else {
          // Prestador: Ação padrão é apenas ver detalhes
          print(
              "Ação Prestador 'concluído': Nenhuma ação backend, chamando simpleSuccess.");
          simpleSuccess();
        }
        break;
  ...
  static void cancelarSolicitacao(BuildContext context, Agendamento agendamento,
    ...
    final bool? confirm = await showDialog<bool>(
      ...
      builder: (BuildContext dialogContext) => AlertDialog(
        ...
        content:
            const Text('Tem certeza que deseja cancelar esta solicitação?'),
        actions: <Widget>[
          TextButton(
  ...
  static void gerenciarSolicitacao(
      ...
      Agendamento agendamento,
      Function(String newStatus) onSuccessWithStatus,
      Function onFailure,
    ...
    showDialog(
      ...
      builder: (BuildContext dialogContext) {
        // Usa dialogContext para ações internas
        return AlertDialog(
          title: const Text('Gerenciar Solicitação'),
          content: Text(
              'Aceitar ou recusar o pedido de ${agendamento.nomeCliente ?? 'Cliente'} para o serviço "${agendamento.nomeServico ?? 'N/A'}"?'),
          actions: [
            TextButton(
              ...
              onPressed: () {
                ...
                aceitarSolicitacao(
                    context, agendamento, onSuccessWithStatus, onFailure);
              },
            ),
            TextButton(
            ...
            TextButton(
              ...
              onPressed: () =>
                  closeLocalDialog(dialogContext), // Apenas fecha este dialog
            ),
          ],
        );
      },
  ...
  static void aceitarSolicitacao(
    ...
    final bool? confirm = await showDialog<bool>(
        ...
        builder: (BuildContext dialogContext) => AlertDialog(
                ...
                content: const Text(
                    'O cliente será notificado para realizar o pagamento PIX (se aplicável).'),
                actions: [
                  TextButton(
                      onPressed: () => Navigator.pop(dialogContext, false),
    ...
    try {
      await AgendamentoService().atualizarStatusAgendamento(
          agendamento.idAgendamento, 'aguardando'); // Muda status
      if (!context.mounted) return;
      _hideLoadingDialog(context);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Solicitação aceita! Aguardando cliente.'),
            backgroundColor: Colors.green),
      );
      onSuccessWithStatus(
          'aguardando'); // Informa ScheduleScreen do sucesso e do novo status
    } catch (e) {
      if (!context.mounted) return;
      _hideLoadingDialog(context);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text('Erro ao aceitar: ${e.toString()}'),
            backgroundColor: Colors.red),
      );
  ...
  static void recusarSolicitacao(BuildContext context, Agendamento agendamento,
      Function(String newStatus) onSuccessWithStatus, Function onFailure) {
    if (!context.mounted) return;
                ...
                children: [
                  ...
                  TextFormField(
                    decoration: const InputDecoration(
                        labelText: 'Motivo *',
                        hintText: 'Ex: Horário indisponível',
            ...
            actions: [
              TextButton(
                  ...
                  onPressed: () => Navigator.pop(
                      dialogContext) // Fecha só o dialog do motivo
                  ),
              TextButton(
                  ...
                  onPressed: () async {
                    if (formKey.currentState!.validate()) {
                      ...
                      Navigator.pop(
                          dialogContext); // Fecha dialog do motivo ANTES de mostrar loading

                      if (!context.mounted)
                        return; // Re-verifica contexto original
                      ...
                      try {
                        ...
                        if (!context.mounted)
                          return; // Verifica após await (catch)
                        _hideLoadingDialog(context);
                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                            content: Text('Erro ao recusar: ${e.toString()}'),
                            backgroundColor: Colors.red));
                        onFailure();
                      }
                    }
  ...
  static Future<void> pagamento(BuildContext context, Agendamento agendamento,
      Function onSuccess, Function onFailure) async {
    if (agendamento.isPix == true &&
        agendamento.precoServico != null &&
        agendamento.precoServico! > 0) {
      // --- Lógica para pagamento PIX ---
      final description =
          'Pagamento: ${agendamento.nomeServico ?? 'Agendamento ${agendamento.idAgendamento}'}';
      print(
          "AgendamentoActions.pagamento: Iniciando fluxo PIX (PaymentScreen) para ${agendamento.idAgendamento}...");

      // Verifica se o contexto ainda é válido antes de navegar
      if (!context.mounted) {
        print(
            "AgendamentoActions.pagamento: Contexto inválido antes de navegar. Abortando.");
      ...
      try {
        ...
        print(
            "AgendamentoActions.pagamento: Retornou do fluxo PaymentScreen/StatusScreen. Resultado final: $pagamentoCompletoBemSucedido");

        // Verifica o resultado e chama o callback apropriado
        if (pagamentoCompletoBemSucedido == true) {
          print(
              "AgendamentoActions.pagamento: Sucesso confirmado pelo fluxo completo. Chamando onSuccess.");
          onSuccess(); // Chama o callback de SUCESSO para ScheduleScreen
        } else {
          // Se retornou false (falha explícita) ou null (cancelamento/volta)
          print(
              "AgendamentoActions.pagamento: Falha ou cancelamento no fluxo. Chamando onFailure.");
          ...
          if (context.mounted) {
            if (pagamentoCompletoBemSucedido == false) {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                  content: Text('Ocorreu uma falha durante o pagamento.')));
            } else {
              // null implica cancelamento
              ScaffoldMessenger.of(context).showSnackBar(
        ...
        print(stacktrace);
        if (context.mounted) {
          // Verifica context.mounted
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text(
                  'Erro inesperado ao iniciar pagamento: ${error.toString()}'),
              backgroundColor: Colors.red));
        }
      ...
      print(
          "AgendamentoActions.pagamento: Iniciando fluxo Não-PIX (instruções).");
      _showNonPixPaymentInstructions(
          context, agendamento, onSuccess, onFailure);
    } else {
      // --- Condição inválida ---
      print(
          "AgendamentoActions.pagamento: Condição inválida para pagamento (isPix: ${agendamento.isPix}, preco: ${agendamento.precoServico})");
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text(
                'Não é possível pagar. Verifique detalhes ou contate suporte.'),
            backgroundColor: Colors.orange));
      }
  ...
  static void _showNonPixPaymentInstructions(BuildContext context,
      Agendamento agendamento, Function onSuccess, Function onFailure) async {
    if (!context.mounted) return;
    _showLoadingDialog(context, message: 'Buscando contato...');
    try {
      final prestadorInfo = await AgendamentoService()
          .obterDetalhesPrestador(agendamento.idPrestador);
      if (!context.mounted) return;
      _hideLoadingDialog(context);
              ...
              content: SingleChildScrollView(
                child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 16),
                      _buildPrestadorInfoRow(
                          'Prestador:', prestadorInfo['nome'] ?? 'N/A'),
                      _buildPrestadorInfoRow('Telefone:', telefone ?? 'N/A'),
                      _buildPrestadorInfoRow(
                          'Forma:', agendamento.formaPagamento ?? 'A combinar'),
                      const SizedBox(height: 16),
                      const Text(
                          'O status será atualizado pelo prestador após confirmar.',
                          style: TextStyle(
      ...
      _hideLoadingDialog(context);
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Erro ao buscar contato: ${e.toString()}'),
        backgroundColor: Colors.red,
      ));
      ...
      child: RichText(
        text: TextSpan(
          style: const TextStyle(
              fontSize: 15, color: Colors.black87), // Estilo padrão
          children: [
            TextSpan(
                text: label,
  ...
  static void verificarPagamento(BuildContext context, Agendamento agendamento,
    ...
    try {
      ...
      final pagamentoConfirmado = await supabase
          .from('pagamentos')
          .select('id_pagamento')
          .eq('id_agendamento', agendamento.idAgendamento)
          .eq('status',
              'confirmado') // Busca status 'confirmado' na tabela pagamentos
          .limit(1)
          .maybeSingle();
      ...
      if (pagamentoConfirmado != null) {
        ...
        if (agendamento.status == 'aguardando') {
          print(
              "Verificar Pagamento: Pagamento confirmado encontrado. Atualizando agendamento ${agendamento.idAgendamento} para 'confirmado'.");
          ...
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text(
                  'Pagamento confirmado! Status do agendamento atualizado.'),
              backgroundColor: Colors.green));
        } else {
          // Pagamento confirmado, mas agendamento já estava 'confirmado' ou outro status
          _hideLoadingDialog(context);
          print(
              "Verificar Pagamento: Pagamento consta como confirmado, agendamento já está como '${agendamento.status}'.");
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text(
                  'Pagamento já estava confirmado no sistema (Status atual: ${agendamento.status}).'),
        ...
        _hideLoadingDialog(context);
        print(
            "Verificar Pagamento: Pagamento para ${agendamento.idAgendamento} ainda não consta como confirmado na tabela de pagamentos.");
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content:
                Text('Pagamento do cliente ainda não confirmado no sistema.'),
            backgroundColor: Colors.orange));
      }
      ...
      _hideLoadingDialog(context);
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Erro ao verificar pagamento: ${e.toString()}'),
          backgroundColor: Colors.red));
    ...
    if (confirmou == true) {
      ...
      try {
        await AgendamentoService()
            .atualizarStatusAgendamento(agendamento.idAgendamento, 'concluído');
        if (!context.mounted) return;
        _hideLoadingDialog(context);
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text('Erro ao finalizar agendamento: ${e.toString()}'),
            backgroundColor: Colors.red));
        onFailure();
      }
    }
  ...
  /// Cliente: Abre dialog para avaliação e envia para o banco de dados.
  ...
  static void verDetalhesRecusa(
      BuildContext context,
      Agendamento agendamento,
      Function onSuccess,
      Function onFailure,
      Function(BuildContext) closeLocalDialog) async {
    ...
    if (motivo.isNotEmpty) {
      showDialog(
        ...
        onSuccess();
      }); // Chama onSuccess quando o dialog é fechado
    } else {
      // Tenta buscar no banco (pode ser redundante)
      _showLoadingDialog(context, message: 'Buscando motivo...');
      try {
        Agendamento detalhes = await AgendamentoService()
            .obterDetalhesAgendamento(agendamento.idAgendamento);
        if (!context.mounted) return;
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text('Erro ao buscar motivo: ${e.toString()}'),
            backgroundColor: Colors.red));
